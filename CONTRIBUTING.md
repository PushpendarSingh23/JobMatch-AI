...
```bash
git clone https://github.com/chamals3n4/JobMatch AI.git
cd JobMatch AI
make setup
make dev
```
...
```bash
git remote add upstream https://github.com/chamals3n4/JobMatch AI.git
...
- **Postgres** on `localhost:5432` (user: `jobmatch-ai`, password: `jobmatch-ai`, db: `jobmatch-ai`)
...
```bash
DATABASE_URL=postgresql://jobmatch-ai:jobmatch-ai@localhost:5432/jobmatch-ai
REDIS_URL=redis://localhost:6379
```

### 3. Set up your Asgardeo M2M application

JobMatch AI uses WSO2 Asgardeo for login and user management. You need your own free tenant.
...
This creates the roles JobMatch AI expects (`super_admin`, `hiring_manager`, `interviewer`) in your tenant and writes the role IDs into `backend/.env`:
