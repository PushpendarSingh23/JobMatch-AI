...
- **Local Postgres + Redis**: `docker-compose.yml` at the repo root runs both as containers (`jobmatch-ai`/`jobmatch-ai`/`jobmatch-ai` for user/password/db on Postgres; Redis with no auth). Not required — Neon/hosted Redis work too — but this is the fastest path for local dev. See `CONTRIBUTING.md` for the full setup flow.
...
- `frontend/.env` — `NEXT_PUBLIC_ASGARDEO_*`, `ASGARDEO_*`, `JOBMATCH_API_URL`, `NEXT_PUBLIC_API_URL`
